...
class RFIDTag(Base):
    ...
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), nullable=False, comment="初めて登録されたタイミング")
...
class ScannedRFID(Base):
    ...
    scanned_at = Column(DateTime, default=lambda: datetime.now(timezone.utc), nullable=False, comment="スキャンされたタイミング")
